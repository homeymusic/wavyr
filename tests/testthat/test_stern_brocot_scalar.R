...
test_that("0.5 returns 1/2", {
  result <- stern_brocot_for_scalar_cpp(1/2, GABOR_UNCERTAINTY)
  expect_equal(result$num, 1)
  expect_equal(result$den, 2)
})

test_that("29 / 42 works", {
  result <- stern_brocot_for_scalar_cpp(29 / 42, GABOR_UNCERTAINTY ^ 2)
...
test_that("if x is less than unceratinty it returns interesting stuff", {
  ...
  result <- stern_brocot_for_scalar_cpp(1, 2)
  expect_equal(names(result), expected_stern_brocot_columns)
  expect_equal(result$num, 1)
  expect_equal(result$den, 1)

  result <- stern_brocot_for_scalar_cpp(50.234, 60)
  expect_equal(names(result), expected_stern_brocot_columns)
  expect_equal(result$num, 50)
  expect_equal(result$den, 1)

  result <- stern_brocot_for_scalar_cpp(0.25, 0.5)
  expect_equal(names(result), expected_stern_brocot_columns)
  expect_equal(result$num, 1)
  expect_equal(result$den, 2)
...
test_that("stern_brocot_for_scalar_cpp function returns correct rational approximation", {
  # Test case 1: Standard input with small uncertainty
  result <- stern_brocot_for_scalar_cpp(2.5, 0.01)
  ...
  result <- stern_brocot_for_scalar_cpp(0.001, 0.0001)
  expect_equal(result$num, 1)
  ...
  result <- stern_brocot_for_scalar_cpp(100.75, 0.1)
  approx_value <- result$num / result$den
  ...
  result <- stern_brocot_for_scalar_cpp(3.333, 0.001)
  approx_value <- result$num / result$den
  expect_true(abs(approx_value - 3.333) <= result$uncertainty)

  # Test case 5: Invalid input (negative x)
  expect_error(stern_brocot_for_scalar_cpp(-2.5, 0.01), "STOP: x must be greater than 0")

  # Test case 6: Invalid input (negative uncertainty)
  expect_error(stern_brocot_for_scalar_cpp(2.5, -0.01), "STOP: uncertainty must be greater than 0")

  # Test case 7: Result should be an integer if x is an integer
  result <- stern_brocot_for_scalar_cpp(3, 0.1)
...
test_that("stern_brocot_for_scalar_cpp does not return zero numerator or denominator", {
  x <- 0.1666667
  uncertainty <- 3.0

  result <- stern_brocot_for_scalar_cpp(x, uncertainty)
...
tritone_result    = stern_brocot_for_scalar_cpp(tritone_ratio, gabor_uncertainty)

P8_ratio          = midi_to_freq(72) / midi_to_freq(60)
P8_result         = stern_brocot_for_scalar_cpp(P8_ratio, gabor_uncertainty)
