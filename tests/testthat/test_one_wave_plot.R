test_that("detailed: wave plot generates correctly with time and space grid", {
  ...
  # Verify that wave_obj has the expected properties
  expect_true(!is.null(wave_obj$frequency_spectrum))
  expect_true(!is.null(wave_obj$wavelength_spectrum))
  expect_true(!is.null(wave_obj$indexed_spectra))
  expect_true(!is.null(wave_obj$composite_amplitude))
  expect_true(!is.null(wave_obj$fundamental_amplitude))

  # Check if composite_amplitude function runs without errors and returns a numeric value
  expect_type(wave_obj$composite_amplitude(0, 0), "double")

  # Check if fundamental_amplitude function runs without errors and returns a numeric value
  expect_type(wave_obj$fundamental_amplitude(0, 0), "double")

  expect_true(inherits(wave_obj, "superposed_wave"))
  expect_true(inherits(wave_obj, "wave"))
